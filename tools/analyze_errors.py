...
def analyze_error_files():
    ...
    for error_file in error_files:
        try:
            ...
            # Store error info
            error_info = {"timestamp": timestamp, "file": filename, "data": error_data}
            ...
            if error_msg:
                ...
                if (
                    ...
                    or "too many" in error_msg_lower
    ...
    for date in sorted(errors_by_date.keys()):
        ...
        if date_errors:
            ...
            if evening_errors:
                ...
                print(
                    f"{date}: First error at {first_error}, Last at {last_error}, "
    ...
    api_limit_msgs = [
        msg
        for msg in error_messages.keys()
        if any(
            keyword in msg.lower() for keyword in ["limit", "quota", "exceeded", "too many", "rate"]
        )
    ]
            ...
            if hours_with_errors:
                ...
                for i in range(len(hours_with_errors) - 1):
                    if hours_with_errors[i + 1] - hours_with_errors[i] > 1:
                        gaps.append(
                            f"{hours_with_errors[i]:02d}:00-{hours_with_errors[i+1]:02d}:00"
                        )
