...
class CachedVehicleClient:
    ...
    def _extract_remote_timestamp(self, data):
        ...
        ev_status = (
            vehicle_status.get("evStatus")
            if isinstance(vehicle_status.get("evStatus"), dict)
            else {}
        )
    ...
    def get_vehicle_data(self):
        ...
        try:
            ...
            if isinstance(e, KeyError) and str(e) == "'vehicleStatus'":
                logger.info(
                    "vehicleStatus not available in API response, using last successful cache"
                )
    ...
    def _classify_error(self, error):
        ...
        if any(
            phrase in error_msg
            for phrase in ["vehicle not found", "no vehicles", "invalid vehicle"]
        ):
            return APIError(
        ...
        if hasattr(vehicle, "data") and "evTripDetails" in vehicle.data:
            ...
            for trip in trip_details:
                ...
                trip_data = {
                    ...
                    "duration": (
                        round(duration_sec / 60) if duration_sec else None
                    ),  # Convert to minutes
                    "average_speed": (
                        round(avg_speed_mph * 1.60934) if avg_speed_mph else None
                    ),  # Convert to km/h
                    "max_speed": (
                        round(max_speed_mph * 1.60934) if max_speed_mph else None
                    ),  # Convert to km/h
                    "idle_time": (
                        round((duration_sec - driving_time_sec) / 60)
                        if duration_sec and driving_time_sec
                        else None
                    ),
                    "trips_count": 1,
                        ...
                        trips.append(
                            {
                                ...
                                "regenerated_energy": (
                                    ...
                                    if hasattr(stat, "regenerated_energy")
                                    else None
                                ),
