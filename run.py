...
import subprocess  # nosec B404 - only runs known local scripts
import sys
import time
import os
...
def main():
    ...
    collector = subprocess.Popen(  # nosec B603 - hardcoded command, no user input
        [sys.executable, "data_collector.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    try:
        ...
        web = subprocess.Popen(  # nosec B603 - hardcoded command, no user input
            [sys.executable, "-m", "src.web.app"], stdout=sys.stdout, stderr=sys.stderr
        )
